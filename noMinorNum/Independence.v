...
Proof.
  ...
  {assert(thread_lookup (tAdd T' (tid, s1, a :: s2, M')) tid (tid, s1, a::s2, M')). 
   econstructor. apply Union_intror. constructor. auto.
   apply uniqueThreadPool with (t':= (tid, s1, a :: s2, M')) in H5. 
...
Proof.
  ...
  {subst. inversion H0; inversion H; subst. unfoldSetEq H9. 
   assert(Ensembles.In(AST.tid*specStack) (specActionsAux
            (tAdd TRB(1::tid, [], s2,t))) (2::tid, s1'++[a])). 
   econstructor. econstructor. econstructor. econstructor. econstructor. 
   inversion H11. eassumption. reflexivity. apply H5 in H8. inversion H8. 
   invertExists. inversion H12. subst. inversion H14; subst. 
   assert(exists X, thread_lookup T' (2::tid) X). eauto. contradiction. 
   inversion H9. invertListNeq. }
  {inversion H4; eauto. } 
...
Proof.
  ...
  {subst. inversion H0; inversion H1; subst. unfoldSetEq H8. 
   assert(Ensembles.In (AST.tid*specStack)(specActionsAux
            (tAdd TRB (1::tid, [], s2, t0))) (2::tid, s1'++[a])).
   ...
   invertExists. inversion H11. subst. inv H13. 
   assert(exists p, thread_lookup T' (2::tid) p). eauto. contradiction. 
   inversion H8. invertListNeq. }
Qed. 
...
Proof. 
  intros. inversion H. inversion H0; subst. unfoldSetEq H4. 
  assert(Ensembles.In (AST.tid * specStack)(specActionsAux (tSingleton (tid, s1, s2, M))) 
                      (tid, s1)). 
...
Proof.
  ...
  {subst. inversion H. inversion H0. subst. unfoldSetEq H8. 
   ...
   inversion H7. invertExists. inversion H11. inv H13. 
   assert(exists y, thread_lookup T' (2::tid) y). eauto. contradiction. 
   inversion H20. invertListNeq. }
