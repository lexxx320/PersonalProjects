...
Theorem readFastForward : forall H T H' T' H'' T'' ds TID x M' E s1 t0 s2 d s1' M N N' N'',
         eraseTrm s1' M N'' ->                     
         ...
         spec_multistep H (tUnion T (tSingleton(TID,unlocked nil,s2,M'))) H'
                        (tUnion T' (tSingleton(TID,unlocked s1, s2, N'))) ->
         spec_multistep H' (tUnion T' (tSingleton(TID,unlocked s1, s2, N'))) H''
                        (tUnion T'' (tSingleton(TID,unlocked(s1'++[rAct x M' E d]),s2,M))) ->
         exists H''' T''' ,
           spec_multistep H (tUnion T (tSingleton(TID,unlocked nil,s2,M'))) H'''
                         (tUnion T''' (tSingleton(TID,unlocked [rAct x M' E d],s2,N''))) /\
           ...
           heap_lookup x H''' = Some(sfull COMMIT ds' SPEC t0 N).
Proof.
Admitted.
...
Proof.
  ...
  {inv H0. inv H3. econstructor; eauto. erewrite unspecHeapRBRead; eauto. 
   ...
   eapply readFastForward in H4; eauto. Focus 2. eapply lookupUnspecFull; eauto. 
   Focus 2. constructor. invertHyp. eapply spec_multi_trans. rewrite spec_multi_unused.
   eassumption. 
