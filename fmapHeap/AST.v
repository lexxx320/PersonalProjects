...
Fixpoint open (k:nat) (u:trm) (t:trm) : trm :=
  match t with
      ...
      |fst_ e => fst_ (open k u e)
      |snd_ e => snd_ (open k u e)
      |_ => t
  end. 
...
|decompFst : forall M E M', ~val M -> decompose M E M' -> 
                            decompose (fst_ M) (fstCtxt E) M'
|decompFstVal : forall M, val M -> decompose (fst_ M) holeCtxt (fst_ M)
|decompSnd : forall M E M', ~val M -> decompose M E M' -> 
                            decompose (snd_ M) (sndCtxt E) M'
|decompSndVal : forall M, val M -> decompose (snd_ M) holeCtxt (snd_ M)
...
Fixpoint fill (c:ctxt) (t:trm) : trm :=
  match c with
      ...
      |fstCtxt E =>fst_ (fill E t)
      |sndCtxt E => snd_ (fill E t)
      |holeCtxt => t
  end. 
