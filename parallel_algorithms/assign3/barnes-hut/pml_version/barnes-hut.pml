...
fun one_step parts =
    ...
    fun f part = calc_accel part tree
    in
      map f parts
    end
end
...
structure Main =
  struct
    ...
    fun readFromFile () =
	let
	    val f = TextIO.openIn "../small_data.txt"
    ...
    fun writeRes vs = 
        let val f = TextIO.openOut "results.txt"
        fun lp i = 
                if i = BarnesHut.size vs
                then TextIO.closeOut f
                else let val (vx, vy) = BarnesHut.sub(vs, i)
                         val _ = TextIO.outputLine("particle " ^ Int.toString i ^ ": (" ^ Double.toString vx ^ ", " ^ Double.toString vy ^ ")\n", f)
                     in lp (i+1)
                     end
        in lp 0
        end
    ...
    fun main (_, args) =
	let
	    val bodiesList = readFromFile()
	    val nIters = dfltI 
	    val bodiesArray = RunPar.runSilent (fn _ => Rope.map bumpParticle (Rope.fromList bodiesList))
	    val result = BarnesHut.one_step bodiesArray
	    val _ = writeRes result
	in
	        result
	end
