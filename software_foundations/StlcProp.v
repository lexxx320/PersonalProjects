...
Proof.
  ...
  {Case "tapp". inversion Ht; subst. apply IHt1 in H2. apply IHt2 in H4. 
   inversion H2.  
   {inversion H4. 
    {inversion H; subst. right. econstructor. constructor. assumption. 
     inversion Ht; subst. inversion H6. inversion Ht. inversion H6. } 
    {inversion H0. right. exists (tapp t1 x0). constructor. assumption. apply H1. }
   }
   {inversion H. right. exists (tapp x0 t2). constructor. assumption. }
  }
  {Case "tif". inversion Ht; subst. apply IHt1 in H3. apply IHt2 in H5. 
   apply IHt3 in H6. right. inversion H3. 
   {inversion H; subst. inversion Ht; subst. inversion H7.
    exists t2. constructor. exists t3. constructor. }
   {inversion H; subst. exists (tif x0 t2 t3). constructor. assumption. }
  }
Qed.
...
Proof.
  ...
  {inversion H; subst. apply IHt1 in H3. apply IHt2 in H5. unfold closed in *. 
   unfold not in *. intros. inversion H0; subst. apply H3 in H4. assumption. 
   apply H5 in H4. assumption. }
  {unfold closed. unfold not. intros. 
   apply free_in_context with (T := T) (Gamma := \empty) in H0. 
   inversion H0. inversion H1. assumption. }
  {unfold closed. unfold not. intros. inversion H0. }
  {unfold closed. unfold not. intros. inversion H0. }
  {inversion H; subst. apply IHt1 in H4. apply IHt2 in H6. apply IHt3 in H7. 
   unfold closed. unfold not. intros. inversion H0; subst; unfold closed in *; unfold not in *. 
   apply H4 in H3. assumption. apply H6 in H3. assumption. apply H7 in H3. assumption. }
Qed.
...
Print has_type. 
Theorem subjectExpansionSTLC : forall t t' Gamma T, 
...
Proof.
  ...
  induction Hmulti.
  {apply progress in Hhas_type. inversion Hhas_type. contradiction. 
   contradiction. }
  {apply preservation with (t' := y0) in Hhas_type. apply IHHmulti; assumption. 
   assumption. }
Qed. 
...
Theorem TypeUniqueness : forall Gamma T1 T2 t,
                           Gamma |- t \in T1 -> Gamma |- t \in T2 -> T1 = T2.
Proof.
...
(** 
Progress: If Gamma |- t : tau, then either t is a value or there exists some t'
          such that t ==> t'

Preservation: If t ==> t' and Gamma |- t : tau, then Gamma |- t' : tau
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]  - Becomes false, (tif ttrue ttrue ttrue) ==> ttrue and zap

      - Progress - remains true

      - Preservation - Becomes false, ttrue ==> zap and \empty |-ttrue : nat, but
                       \empty |- zap : nat -> nat
...
(** Suppose instead that we add a new term [foo] with the following reduction rules:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step] - becomes false (\x:A.x) ttrue ==> ttrue and
                               (\x:A.x) ttrue ==> foo ttrue

      - Progress  - remains true

      - Preservation - becomes false, (\x:a.x) ==> foo, but foo doesn't type check
...
(** Suppose instead that we remove the rule [ST_App1] from the [step]
    ...
    false, give a counterexample.
      ...
      - Progress - becomes false ((\x : nat. (\y : nat. y) x) 3) 5 has type nat, but can't step
...
(** Suppose instead that we add the following new rule to the typing relation:
    ...
    false, give a counterexample.
      ...
      - Preservation - becomes false (\x : bool. \y : bool. x) tfalse has type bool, but 
                               (\y : bool. tfalse) has type bool -> bool
...
(** Suppose instead that we add the following new rule to the typing relation:
    ...
    false, give a counterexample.
      ...
      - Progress - becomes false, (ttrue ttrue) is well typed, but cannot step
...
(** Suppose we add the following new rule to the typing
    ...
    false, give a counterexample.
      ...
      - Preservation - \empty |- (\y : nat. \x : Bool. 10) 12 : Bool -> nat, but
                       \empty |- \x : Bool. 10 : Bool
