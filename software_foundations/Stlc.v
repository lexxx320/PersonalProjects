...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop := 
  ...
  | s_var2 : forall y, x <> y -> substi s x (tvar y) (tvar y)
  | s_abs1 : forall T t1, substi s x (tabs x T t1) (tabs x T t1)
  | s_abs2 : forall y T t1 t1', x <> y -> substi s x t1 t1' -> 
  ...
  |s_if : forall t1 t2 t3 t1' t2' t3', substi s x t1 t1' ->
                                       substi s x t2 t2' ->
                                       substi s x t3 t3' ->
...
Proof.
  intros. split; intros. 
  Case "->".
    generalize dependent t'. induction t; simpl in *; intros. 
    {destruct (eq_id_dec x0 i); subst. 
     {constructor. }{apply s_var2. assumption. }
    }
    {rewrite <- H. constructor. apply IHt1. reflexivity. 
     apply IHt2. reflexivity. }
    {rewrite <- H. destruct (eq_id_dec x0 i); subst. 
     {apply s_abs1. } 
     {apply s_abs2. assumption. apply IHt. reflexivity. }
    }
    {rewrite <- H. constructor. }
    {rewrite <- H. constructor. }
    {rewrite <- H. constructor. apply IHt1. reflexivity. 
     apply IHt2. reflexivity. apply IHt3. reflexivity. }
  Case "<-".
    induction H; simpl.  
    {rewrite eq_id. reflexivity. }
    {rewrite neq_id. reflexivity. assumption. }
    {rewrite eq_id. reflexivity. }
    {rewrite neq_id. rewrite IHsubsti. reflexivity. assumption. }
    {subst. reflexivity. }
...
Theorem RecNotEq : forall t1 t2, ~(t1 = TArrow t1 t2). 
Proof.
  induction t1; unfold not; intros; try inversion H. 
  {unfold not in *. apply IHt1_1 in H1. assumption. }
  Qed. 
...
Proof.
  ...
  clear H. inversion H0. inversion H; subst. 
  inversion H6; subst. inversion H4; subst. 
  inversion H7; subst. rewrite H5 in H3. inversion H3.
  apply RecNotEq in H2. assumption. Qed. 
