...
STRING | COMMENT | STRINGFLUSH | INITIAL
...
val MaxString = 256
val StrLenError = ref false
val StrArray = Array.array(MaxString,#"\000")
val StrInd = ref 0
...
fun eof () =
  ...
  in
     if (!commentlevel <> 0) 
     then ( (err (!startpos,pos) "unterminated comment"); commentlevel := 0)
     else 
       if !instring
       then (if (!escapepos <> ~1) 
             then ( (err (!escapepos,pos) 
                        "unterminated format sequence in string") ;
                    escapepos := ~1)
             else ();
             (err (!startpos,pos) "unterminated string"); instring := false)
       else ();
     Tokens.EOF(pos,pos)
  end

fun AddtoStrArray str n = 
      if (not (!StrLenError))
      then 
         let val chars = explode str
             fun addto _ 0 = ()
               | addto (hd :: tl) n = 
                   ( Array.update(StrArray,!StrInd,hd);
                     (inc StrInd);
                     (addto tl (n - 1)))
         in (addto chars 
                   (if ((n + !StrInd) > MaxString)
                    then 
                      ( StrLenError := true;
                        (err (!startpos,!startpos+(!StrInd)+n)
                            ("string too long --- truncating to max length."));
                        (MaxString - !StrInd) )
                    else n))
         end
      else ()
...
fun ExtractStr () =
       let 
         fun addto accum =
               if (!StrInd = 0) then (implode accum)
               else 
	         (dec StrInd; (addto (Array.sub(StrArray,!StrInd) :: accum)))
       in addto []
       end
      ...
      in
        ...
        (case Int.fromString(yytext) of
          NONE => (ErrorMsg.impossible
                    ("cannot convert to integer:  " ^ yytext))
        | SOME d => Tokens.INT(d, yypos, yypos+size(yytext)))
      end
...
fun yyAction50 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN INITIAL;
                   instring := false; StrLenError := false;
                   Tokens.STRING(ExtractStr(), !startpos, yypos+1)))
fun yyAction51 (strm, lastMatch : yymatch) = (yystrm := strm;
      (inc lineNum; linePos := yypos :: !linePos;
		   err (!startpos, yypos)
		     ("string ended with newline:  " ^ (ExtractStr ()));
                   YYBEGIN INITIAL; instring := false; StrLenError := false;
                   Tokens.STRING(ExtractStr(), !startpos, yypos)))
fun yyAction52 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        ((AddtoStrArray yytext (size yytext));
                      continue())
      end
fun yyAction53 (strm, lastMatch : yymatch) = (yystrm := strm;
      ((AddtoStrArray "\n" 1); continue()))
fun yyAction54 (strm, lastMatch : yymatch) = (yystrm := strm;
      ((AddtoStrArray "\t" 1); continue()))
fun yyAction55 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        ((AddtoStrArray (str (chr (Word.toInt 
                                    (Word.andb
                                        ((Word.fromInt 
                                            (ord (String.sub (yytext, 2)))),
                                        0wx1f))))) 1);
       continue())
      end
fun yyAction56 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        ((case Int.fromString(substring(yytext,1,3)) of
           NONE => ErrorMsg.impossible
                     ("unable to convert " ^ yytext ^ " escape")
         | SOME d => if (d > 127) 
                     then (err (yypos,yypos+4) 
                              "Illegal character code in string")
                     else (AddtoStrArray (str (chr d)) 1));
       continue())
      end
...
fun yyAction64 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        ( (err (yypos,yypos+1) ("illegal character " ^ yytext)); continue())
      end
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction1(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction1(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction9(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction9, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction9(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction9, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction9, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction3(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction3, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction3(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction3, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction3, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction13(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction13, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction13(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction13, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction13, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction17(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction17, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction17(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction17, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction17, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction5(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction5, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction5(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction5, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction5, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction6(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction6, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction6(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction6, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction6, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction12(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction12, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction12(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction12, yyNO_MATCH))
            else if inp = #"`"
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction8(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction8, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction8(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction8, yyNO_MATCH))
            else if inp = #"`"
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction2(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction2, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction2(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction2, yyNO_MATCH))
            else if inp = #"`"
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction7(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction7, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction7(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction7, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction7, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction14(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction14, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction14(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction14, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction14, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction15(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction15, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction15(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction15, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction15, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction4(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction4, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction4(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction4, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction4, yyNO_MATCH))
       ...
       of NONE => yyAction41(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then if inp = #"`"
                  then yyAction41(strm, yyNO_MATCH)
                  else yyQ106(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
            else if inp <= #"z"
              then yyQ68(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
              else yyAction41(strm, yyNO_MATCH)
      (* end case *))
       ...
       of NONE => yyAction41(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then yyQ105(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
            else if inp < #"e"
              then if inp = #"`"
                  then yyAction41(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction11(strm, yyNO_MATCH)
                      else yyQ68(strm', yyMATCH(strm, yyAction11, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction11(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction11, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ68(strm', yyMATCH(strm, yyAction11, yyNO_MATCH))
       ...
       of NONE => yyAction41(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then yyQ111(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
            else if inp < #"y"
              then if inp = #"`"
                  then yyAction41(strm, yyNO_MATCH)
                  else yyQ68(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
       ...
       of NONE => yyAction41(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then if inp = #"`"
                  then yyAction41(strm, yyNO_MATCH)
                  else yyQ110(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
            else if inp <= #"z"
              then yyQ68(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
              else yyAction41(strm, yyNO_MATCH)
      (* end case *))
...
fun yyQ47 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction34(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #">"
              then yyQ114(strm', yyMATCH(strm, yyAction34, yyNO_MATCH))
            else if inp < #">"
              then if inp = #"="
...
fun yyQ45 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction19(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"="
              then yyQ115(strm', yyMATCH(strm, yyAction19, yyNO_MATCH))
...
fun yyQ118 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction0(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"\v"
              then yyQ118(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
            else if inp < #"\v"
              then if inp = #"\t"
                  then yyQ118(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
...
fun yyQ24 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction46(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ25(strm', yyMATCH(strm, yyAction46, yyNO_MATCH))
...
fun yyQ23 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction46(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ26(strm', yyMATCH(strm, yyAction46, yyNO_MATCH))
              else yyAction46(strm, yyNO_MATCH)
      (* end case *))
       ...
       of NONE => yyAction60(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"`"
              ...
              then if inp <= #"?"
                  then yyAction60(strm, yyNO_MATCH)
                  else yyQ17(strm', yyMATCH(strm, yyAction60, yyNO_MATCH))
            else if inp <= #"z"
...
fun yyQ12 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction60(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"0"
              then yyQ18(strm', yyMATCH(strm, yyAction60, yyNO_MATCH))
            else if inp < #"0"
              then yyAction60(strm, yyNO_MATCH)
            else if inp <= #"9"
              then yyQ18(strm', yyMATCH(strm, yyAction60, yyNO_MATCH))
       ...
       of NONE => yyAction64(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"#"
              ...
              then if inp = #"\r"
                  then yyQ9(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
                else if inp < #"\r"
                  then if inp = #"\n"
                      ...
                      then if inp = #"\t"
                          then yyQ9(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
                          else yyQ8(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
                    else if inp = #"\v"
                      then yyQ9(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
                      else yyQ8(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
                else if inp = #"!"
              ...
              then yyQ16(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
              else yyQ8(strm', yyMATCH(strm, yyAction64, yyNO_MATCH))
      (* end case *))
       ...
       of NONE => yyAction52(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"\""
              ...
              then if inp = #"\n"
                  then yyAction52(strm, yyNO_MATCH)
                  else yyQ20(strm', yyMATCH(strm, yyAction52, yyNO_MATCH))
            else if inp = #"\\"
              then yyAction52(strm, yyNO_MATCH)
              else yyQ20(strm', yyMATCH(strm, yyAction52, yyNO_MATCH))
      (* end case *))
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | STRINGFLUSH => yyQ2(!(yystrm), yyNO_MATCH)
