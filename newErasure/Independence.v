...
Proof.
  ...
  {subst. inversion H0; inversion H; subst. unfoldSetEq H9. 
   assert(Ensembles.In(AST.tid*specStack) (specActionsAux
            (tAdd TRB(tid, [], s2,t))) (Tid(maj,min) tid', s1'++[a])). 
   econstructor. econstructor. econstructor. econstructor. econstructor. 
   inversion H11. eassumption. reflexivity. apply H5 in H8. inversion H8. 
   invertExists. inversion H12. inversion H21. 
   assert(exists X, thread_lookup T' (Tid (maj, min) tid') X). econstructor. 
   eapply hit. eassumption. reflexivity. contradiction. inversion H23. 
   destruct s1'; inversion H27. }
  {inversion H4; eauto. } 
...
Proof.
  ...
  {subst. inversion H0; inversion H1; subst. unfoldSetEq H8. 
   assert(Ensembles.In (AST.tid*specStack)(specActionsAux
            (tAdd TRB (tid, [], s2, t0))) (Tid(maj, min)tid', s1'++[a])).
   ...
   invertExists. inversion H11. subst. inversion H20. 
   {assert(exists p, thread_lookup T' (Tid(maj, min) tid') p). econstructor.
    eapply hit. eassumption. reflexivity. inversion H13. contradiction. }
...
Proof.
  ...
  {subst. inversion H. inversion H0. subst. unfoldSetEq H8. 
   ...
   inversion H7. invertExists. inversion H11. inversion H20. 
   assert(exists y, thread_lookup T' (Tid(maj, min) tid') y). econstructor. 
   eapply hit. eassumption. reflexivity. contradiction. inversion H22. 
   destruct s1'; inversion H26. }
