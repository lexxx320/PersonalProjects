...
|eraseSpecReturn : forall e e' tid N0 N', 
                     eraseTerm e e' -> eraseTerm N0 N' ->
                     eraseTerm  (specReturn e tid N0)
...
|tEraseRead : forall tid min s1 s1' s2 x M M' M'' E,
               s1 = s1' ++ [rAct x min M'] -> eraseTerm M' M'' ->
               decompose M' E (get (fvar x)) -> 
               eraseThread (tid, s1, s2, M) (pSingleton M'')
|tEraseWrite : forall tid min M M' M'' x s1 s2 s1' E N,
                s1 = s1' ++ [wAct x min M'] -> eraseTerm M' M'' ->
...
|tEraseSpecRet : forall tid tid' min M M' M'' s1 s2 s1' E N N',
                s1 = s1' ++ [specRetAct tid' min M'] -> eraseTerm M' M'' ->
...
Proof.
  ...
  {inversion H; subst. inversion H0; subst. cleanup. 
   ...
   end. Focus 2. eassumption. destruct EX1. destruct p. destruct p. destruct t0. 
   destruct p. inversion H4; subst. cleanup. econstructor. econstructor. eassumption.
...
|eraseCtxtSpec : forall E E' N0 N0' tid,
                   eraseContext E E' -> eraseTerm N0 N0' -> 
                   eraseContext (specReturnCtxt E tid N0)
...
Proof.
  ...
  {inversion H; subst. inversion H0; subst. cleanup. inversion H3; subst. 
   ...
   eapply termErasureDeterminism in H11; eauto. subst. auto. }
...
Proof.
  ...
  {inversion H0; subst. inversion H; subst; destruct tid; destruct p;
