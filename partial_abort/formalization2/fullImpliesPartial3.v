...
|commitAbortOK : forall S L L'  S' S'' H' e0 e e' L'' e'',
                    validate S L H S' (commit H') -> 
                    validate S L' H S'' (abort e'' L'') -> postfix L L' ->
                    OK H (Some(S,e0),L,e) (Some(S,e0),L',e')
|abortAbortOK : forall S L L' S' S'' eAbort eAbort' LAbort LAbort' e0 e e',
...
Theorem abortNewTermIsGet : forall S L H C e L',
                              validate S L H C (abort e L') -> logWF L ->
                              exists l E, decompose e E (get (loc l)) /\
                                          logLookup L' l = None. 
Proof.
  intros. dependent induction H0. 
  {inv H1. apply IHvalidate in H5. invertHyp. econstructor. econstructor. eauto. 
   eauto. }
  {inv H3. exists l. exists E. split. apply fillGet. auto. }
Qed. 
...
Theorem validateInvalidOneStep : forall S L H C L' e', 
                                   validate S L H C (abort e' L') -> logWF L ->
                                   exists E l v S', decompose e' E (get (loc l)) /\
                                                    lookup H l = Some(v, S') /\ S' >= S /\
                                                    logLookup L' l = None. 
Proof.
  intros. dependent induction H0.
  {inv H1; eauto. }
  {inv H3. econstructor. econstructor. econstructor. econstructor. split.
   eapply fillGet. eauto. }
Qed. 
...
Proof.
  ...
  {inv H2. 
   {copy H4. eapply validateInvalidOneStep in H4. invertHyp. inv H0; decompSame; invertEq. 
    {apply validateValidate in H2. invertHyp. eapply multistepAbortCommit in H1; eauto. 
     contradiction. eapply validateAbortRead; eauto. rewrite H4 in H16. inv H16. auto. }
    {rewrite H15 in H9. inv H9. }
    auto. 
   }
   {eapply trans_stepDeterministic in H0; eauto. subst. 
    inv H6; eapply IHtrans_multistep; eauto. }
  }
Qed. 

Theorem postfixTrans : forall (A:Type) (l1 l2 l3 : list A), 
                         postfix l1 l2 -> postfix l2 l3 -> 
                         postfix l1 l3. 
Proof.
  intros. unfold postfix in *. invertHyp. exists (x++x0). 
...
Proof.
  ...
  {inv H6. 
   {left. econstructor. split. eauto. inv H1; try solve[econstructor; eauto; constructor]. 
    {destruct (lt_dec S'0 S). 
     {eapply commitCommitOK. eapply validateCommitRead; eauto.
      eapply validateCommitRead; eauto. constructor. }
     {eapply abortAbortOK. eapply validateAbortRead; eauto ; omega. 
      eapply validateAbortRead; eauto ; omega. }
    }
    {eapply commitCommitOK. eapply validateWrite; eauto. 
     eapply validateWrite; eauto. constructor. }
   }
   {eapply trans_stepDeterministic in H1; eauto. subst. right.
    inv H0; try solve[eapply commitCommitOK; eauto].  
    {destruct (lt_dec S'0 S). 
     {eapply commitCommitOK; eauto. eapply validateCommitRead; eauto. }
     {eapply multistepAbortCommit in H7. contradiction.
      eapply validateAbortRead; eauto. omega. eauto. }
    }
    {eapply commitCommitOK; eauto. eapply validateWrite; eauto. }
   }
  }
  {right. inv H1; try solve[eapply commitAbortOK; eauto]. 
   {destruct (lt_dec S'0 S).  
    {inv H2. Focus 2. eapply validateSameAns in H4; eauto. contradiction. 
     ...
     eapply trans_multistep_trans. eapply H14. econstructor. eapply t_readStep; eauto.
     constructor. copy H7. eapply multistepPostfix in H1; eauto. 
     Focus 2. eapply validateCommitRead; eauto. eapply commitAbortOK. 
     eapply validateCommitRead; eauto. eauto. assert(postfix L'' L'). 
     apply abortLogPostfix in H7. invertHyp. unfold postfix. exists x. auto. 
     eapply postfixTrans; eauto. }
    {eapply abortAbortOK; eauto. eapply validateAbortRead; eauto. omega. }
   }
   {inv H2. Focus 2. eapply validateSameAns in H4; eauto. contradiction. 
    inv H3. eapply validateSameAns in H5; eauto. contradiction.
    eapply abortSameAns in H5; eauto. invertHyp.
    assert(trans_multistep H (Some (S, e0), [], e0) (Some (S, e0), writeItem l v::L, fill E unit)). 
    eapply trans_multistep_trans. eapply H13. econstructor. eapply t_writeStep; eauto.
    constructor. copy H7. eapply multistepPostfix in H1; eauto. 
    Focus 2. eapply validateWrite; eauto. eapply commitAbortOK. 
    eapply validateWrite; eauto. eauto. assert(postfix L'' L').
    apply abortLogPostfix in H7. invertHyp. unfold postfix. exists x. auto. 
    eapply postfixTrans; eauto. 
   }
  }
  ...
  {inv H0. 
   {eapply IHtrans_multistep; eauto. inv H2. 
    {eapply multistepAbortCommit in H1; eauto. contradiction.
     eapply validateAbortPropogate; eauto. }
    {inv H0; decompSame; invertEq. rewrite H13 in H16. inv H16. eauto. 
     rewrite H15 in H12. inv H12. }
   }
   {eapply IHtrans_multistep; eauto. inv H2. 
    {eapply multistepAbortCommit in H1; eauto. contradiction. }
...
Theorem postFixAborts : forall L L' L'' S S' H H' e',
                          validate S L H S' (commit H') ->
                          validate S L' H S' (abort e' L'') ->
                          postfix L' L -> False. 
Proof.
  intros. dependent induction H0. 
  {unfold postfix in H2. invertHyp. destruct x. simpl in H. subst. inv H1. 
   inv H. }
  {unfold postfix in *. invertHyp. destruct x.
   {simpl in *. subst. inv H3. eapply validateSameAns in H2; eauto. 
    rewrite H0 in H14. inv H14. omega. }
   {inv H5. eauto. }
  }
  {unfold postfix in *. invertHyp. destruct x.
   {simpl in *. subst. inv H1. eapply validateSameAns in H0; eauto. }
   {inv H3. eauto. }
  }
Qed. 
  ...
  {inv H3.  
   ...
   {inv H1. InTac. invertHyp. simpl in H1. eapply validateHeapExtensionC in H5; eauto. 
    ...
    {invertHyp. eapply validateHeapExtensionC in H6; eauto. inv H6. 
     ...
     {invertHyp. constructor. eapply commitAbortOK; eauto.
      eapply transMultiLogPostfix in H7. auto. }
    }
    {eapply validateHeapExtensionC in H6; eauto. inv H6. 
     {invertHyp. copy H7. eapply transMultiLogPostfix in H10.
      eapply postFixAborts in H5; eauto. contradiction. }
   ...
   {inv H1. InTac. invertHyp. simpl in H1. eapply validateHeapExtensionC in H5; eauto. inv H5. 
    {invertHyp. eapply validateHeapExtensionA in H6; eauto. inv H6. 
     {constructor. eapply commitAbortOK; eauto. }
     {constructor. invertHyp. eapply commitAbortOK; eauto. }
    }
    {invertHyp. eapply validateHeapExtensionA in H6; eauto. inv H6. 
     {constructor. eapply abortAbortOK; eauto. }
     {invertHyp. constructor. eapply abortAbortOK; eauto. }
    }
   }
...
Proof.
  ...
  {inv H2. eapply trans_stepOK in H0; eauto. inv H0. 
   ...
   inv H1. InTac. invertHyp. auto. inv H3. InTac. invertHyp. auto.  
  }
  ...
  {inv H0. copy H1. eapply wfPar_r in H1. eapply IHf_step in H8; auto. invertHyp. 
   econstructor. split. eapply p_multi_R. eassumption. constructor. auto.
   copy H3. eapply stampHeapMonotonic in H3; auto. invertHyp. copy H2. inv H2. 
   eapply poolOKExtension; eauto. apply wfPar_l in H0. auto. apply wfPar_l in H3. 
   auto. apply wfPar_r in H2. auto. apply wfPar_r in H2. auto. }
  ...
  {inv H2. inv H5. 
   ...
   {eapply abortSameAns in H0; eauto. invertHyp. inv H3. InTac.
    ...
    eapply p_abortStep; eauto. constructor. constructor. copy H8. 
    eapply validateValidate in H8. invertHyp. eapply commitCommitOK. 
  ...
  {inv H3. inv H6. 
   ...
   {inv H1. InTac. invertHyp. inv H3. Focus 2. 
    eapply validateSameAns in H0; eauto. contradiction. inv H4. clear H6. InTac. 
    invertHyp. inv H4. eapply validateSameAns in H11; eauto. contradiction. 
    

 eapply inconsistentTraces in H11; eauto. 
    contradiction. eauto. }
   {eapply validateSameAns in H0; eauto. contradiction. }
  }
