...
Theorem skip_right: forall c,
  cequiv 
    ...
    c.
Proof. 
  intros c. unfold cequiv. intros. 
  split. 
  Case "->". 
    intros. inversion H. 
    subst. inversion H5. subst. assumption. 
  Case "<-".
    intros. econstructor. apply H. constructor. 
  Qed. 
...
Proof.
  ...
  Case "->".
    intros. inversion H0. 
      {subst. unfold bequiv in H. rewrite -> H in H6. inversion H6. }
      {subst. apply H7. }
  Case "<-".
    intros. apply E_IfFalse. 
    {unfold bequiv in H. rewrite H. reflexivity. }
    {apply H0. }
Qed. 



Theorem negBothSides : forall b1 b2, negb b1 = b2 -> b1  = negb b2.
Proof. 
  intros. destruct b1; simpl. destruct b2; try reflexivity. 
  inversion H. rewrite <- H. reflexivity. Qed. 
...
Proof.
  intros. split. 
  {intros. inversion H. 
   {subst. apply E_IfFalse. simpl. rewrite -> H5. reflexivity. apply H6. }
   {subst. apply E_IfTrue. simpl. rewrite -> H5. reflexivity. apply H6. }
  }
  {intros. inversion H. 
   {subst. apply E_IfFalse. simpl in *. apply negBothSides in H5. rewrite H5. 
    reflexivity. apply H6. }
   {subst. apply E_IfTrue. simpl in *. apply negBothSides in H5. rewrite H5. 
    reflexivity. apply H6. }
  }
Qed. 
...
(* if the condition of a while loop is equivalent to true, it will never
   terminate*)
*)
(** [] *)
...
Proof. 
  intros. split. 
  {intros. apply WHILE_true_nonterm with (c := c) (st := st) (st' := st') in H. 
   unfold not in *. apply H in H0. inversion H0. }
  {intros. assert(~(WHILE BTrue DO SKIP END) /st || st'). 
...
Proof.
  ...
  {intros. inversion H. subst. inversion H2. subst.
   apply E_Seq with (st' := st'1). apply H3. 
   apply E_Seq with (st' := st'0). apply H7. apply H5. }
  {intros. inversion H. inversion H5. subst. 
   apply E_Seq with (st' := st'1). apply E_Seq with (st' := st'0).
   apply H2. apply H8. apply H11. }
Qed. 
...
Proof.
  intros. split; intros. 
  {inversion H0; subst. assert(st' = update st' X (st' X)).
   apply functional_extensionality. intros. rewrite update_same.
   reflexivity. reflexivity. rewrite -> H1 at 2. apply E_Ass. 
  ...
  {inversion H0; subst. replace (update st X (aeval st e)) with st. 
   ...
   rewrite update_same. reflexivity. unfold aequiv in H. 
