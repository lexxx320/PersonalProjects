...
Theorem optimize_0mulSound : forall a : aexp, aeval (optimize_0mul a) = aeval a.
Proof.
  intros.
  induction a.
  {simpl. reflexivity. }
...
Inductive bevalR : bexp -> bool -> Prop :=
  |beval_true : bevalR BTrue true
  |beval_false : bevalR BFalse false
  |beval_eq : forall e1 e2 n1 n2, (e1 || n1) -> (e2 || n2) -> bevalR (BEq e1 e2) (beq_nat n1 n2)
  |beval_le : forall e1 e2 n1 n2, (e1 || n1) -> (e2 || n2) -> bevalR (BLe e1 e2) (ble_nat n1 n2)
  |beval_not : forall e1 b, bevalR e1 b -> bevalR (BNot e1) (negb b)
  |beval_and : forall e1 e2 b1 b2, bevalR e1 b1 -> bevalR e2 b2 -> bevalR (BAnd e1 e2) (b1 && b2).

Theorem bevalIffBevalR : forall b e, beval e = b <-> bevalR e b.
Proof.
  split. generalize dependent b.
  Case "->".
  induction e.
    SCase "e = BTrue". intros. destruct b.
      {apply beval_true. }
      {inversion H. }
    SCase "e = BFalse". intros. destruct b.
      {inversion H. }
      {apply beval_false. }
    SCase "e = BEq a a0". intros.
      {simpl in H. rewrite <- H. apply beval_eq. apply aeval_iff_aevalR'. reflexivity. 
       apply aeval_iff_aevalR'. reflexivity. }
    SCase "e = BLe a a0". intros.
      {simpl in *. rewrite <- H. apply beval_le. apply aeval_iff_aevalR'; reflexivity.
       apply aeval_iff_aevalR'; reflexivity. }
    SCase "e = BNot". intros.
      {simpl in *. rewrite <- H. apply beval_not. apply IHe. reflexivity. }
    SCase "e = BAnd a a0".
      {intros. simpl in *. rewrite <- H. apply beval_and. apply IHe1. reflexivity.
       apply IHe2. reflexivity. }
  Case "<-".
     intros. induction H.
     {reflexivity. }
     {reflexivity. }
     {simpl. apply aeval_iff_aevalR' in H. apply aeval_iff_aevalR' in H0. rewrite -> H. rewrite -> H0.
      reflexivity. }
     {simpl. apply aeval_iff_aevalR' in H. apply aeval_iff_aevalR' in H0. rewrite -> H. rewrite -> H0. 
      reflexivity. }
...
Proof.
  ...
  {apply ex_falso_quodlibet. unfold not in H.
   apply H. apply e. }
  {reflexivity. }
Qed.
...
Proof.
  intros.
  unfold update. apply neq_id. apply H. Qed.
...
Proof.
  intros. 
  unfold update. apply neq_id. unfold not. intros. inversion H. Qed.
...
Theorem XYZ : forall st st' n1 n2, st X = n1 -> st Y = n2 -> XtimesYinZ / st || st' ->
                                   st' Z = n1 * n2.
Proof.
  intros. 
  inversion H1. subst. simpl. unfold update. rewrite -> eq_id.
  reflexivity. Qed.
...
Inductive no_whilesR: com -> Prop :=
 |no_while_skip : no_whilesR CSkip
 |no_while_ass : forall x e, no_whilesR (CAss x e)
 |no_while_seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CSeq c1 c2)
 |no_whiles_if : forall b c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CIf b c1 c2).
...
Proof. 
  ...
  {intros. inversion H. symmetry in H1. apply andb_true_eq in H1.
   inversion H1. symmetry in H0. apply IHp1 with (st := st) in H0.
   inversion H0. symmetry in H2. apply IHp2 with (st := x) in H2.
   inversion H2. exists x0. apply E_Seq with (st' := x). apply H3.
   apply H4. }
  {intros. destruct (beval st b) eqn : H1. 
   {simpl in *. symmetry in H. apply andb_true_eq in H. inversion H.
    symmetry in H0. apply IHp1 with (st := st) in H0. inversion H0.
    exists x. apply E_IfTrue. apply H1. apply H3. }
   {simpl in *. symmetry in H. apply andb_true_eq in H. inversion H.
    symmetry in H2. apply IHp2 with (st := st) in H2. inversion H2.
    exists x. apply E_IfFalse. apply H1. apply H3. }
  }
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match stack, prog with
      |_, SPush n :: is => s_execute st (n::stack) is
      |_, SLoad id :: is => s_execute st (st id::stack) is
      |n1::n2::ns, SPlus::is => s_execute st (n2+n1::ns) is
      |n1::n2::ns, SMinus::is => s_execute st (n2-n1::ns) is
      |n1::n2::ns, SMult::is => s_execute st (n2*n1::ns) is
      |_, nil => stack
      |_,_ => nil
  end.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Ass : forall st x e, CAss x e / st || SContinue / update st x (aeval st e)
  | E_SeqBreak : forall st st' s1 s2, s1 / st || SBreak / st' -> 
                                      CSeq s1 s2 / st || SBreak / st'
  ...
  |E_IfFalse : forall st st' b stat s1 s2, beval st b = false ->
                                           s2 / st || stat / st' ->
                                           CIf b s1 s2 / st || stat / st'
  |E_WhileDone : forall st b s, beval st b = false ->
                                          CWhile b s / st || SContinue / st
  |E_WhileBreak : forall st st' b s, beval st b = true ->
                                          s / st || SBreak / st' ->
                                          CWhile b s / st || SContinue / st'
  |E_WhileCont : forall st st' st'' stat b s, beval st b = true ->
                                          s / st || SContinue / st' ->
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  [ Case_aux c "E_Skip";
    ...
    Case_aux c "E_WhileDone";
    Case_aux c "E_WhileBreak";
    Case_aux c "E_WhileBreak";
    Case_aux c "E_WhileCont"
  ].
...
Theorem while_continue : forall b c st st' s,
  ...
  s = SContinue.
Proof.
  intros. inversion H. reflexivity. reflexivity. reflexivity. Qed.
...
Proof.
  intros. inversion H0. 
  {apply E_WhileBreak. subst. apply H. apply E_Break. }
  {apply E_WhileBreak. apply H. apply E_SeqBreak. apply H1. }
  {apply E_WhileBreak. apply H. apply E_Seq with (st' := st'0).
   apply H1. apply H2. }
  {apply E_WhileBreak. apply H. apply E_IfTrue. apply H1.
   apply H2. }
