...
Proof.
  ...
  {intros. inversion H0. 
   ...
   {subst. apply IHstep in H4. subst. reflexivity. }
   {subst. inversion H3. rewrite <- H1 in H. inversion H. }
  }
  {intros. inversion H1. 
   {subst. inversion H0. }
   {subst. inversion H. rewrite <- H2 in H5. inversion H5. }
...
Proof.
  exists (P (C 1) (C 2)).
  split. apply v_funny. unfold normal_form. unfold not. intros. 
...
Proof.
  ...
  {unfold normal_form. unfold not. intros. inversion H. 
...
Proof.  
  ...
  {left. apply v_false. }
  {inversion IHt1. 
   {right. inversion H. exists t2. constructor. exists t3. constructor. }
   {right. inversion H. exists (tif x t2 t3). constructor. apply H0. }
  }
Qed.
...
Proof.
  ...
  induction H. 
  {intros. inversion H0. 
   {reflexivity. }{subst. inversion H4. }
  }
  {intros. inversion H0. 
   {reflexivity. }{subst. inversion H4. }
  }
...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_If : forall t1 t1' t2 t3,
      t1 ==> t1' ->
      tif t1 t2 t3 ==> tif t1' t2 t3
  | ST_SSTrue : forall t1 t2 t3,
                  t2 = t3 -> tif t1 t2 t3 ==> t2
...
Proof.
  ...
  apply ST_SSTrue. reflexivity. Qed. 
...
Theorem ShortCircuitDeterministic : ~(deterministic step). 
Proof.
  unfold deterministic. 
  unfold not. intros. assert(tif (tif ttrue ttrue ttrue) ttrue ttrue ==> ttrue). 
  {apply ST_SSTrue. reflexivity. }
  assert(tif (tif ttrue ttrue ttrue) ttrue ttrue ==> tif ttrue ttrue ttrue). 
  {constructor. constructor. }
  assert( tif (tif ttrue ttrue ttrue) ttrue ttrue ==> ttrue ->
          tif (tif ttrue ttrue ttrue) ttrue ttrue ==> tif ttrue ttrue ttrue).
  intros. apply H1. apply H with (y1 := ttrue) in H2. inversion H2. 
  apply ST_SSTrue. reflexivity. apply ST_SSTrue. reflexivity. Qed.
...
Proof. Print multi.  
  ...
  repeat constructor. apply multi_refl. Qed.
