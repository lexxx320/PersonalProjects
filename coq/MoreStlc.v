...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tnat n => tnat n
  | tsucc n => tsucc (subst x s n)
  | tpred n => tpred (subst x s n)
  | tmult e1 e2 => tmult (subst x s e1) (subst x s e2)
  | tif0 e1 e2 e3 => tif0 (subst x s e1) (subst x s e2) (subst x s e3)
  | tpair e1 e2 => tpair (subst x s e1) (subst x s e2)
  | tfst e => tfst (subst x s e)
  | tsnd e => tsnd (subst x s e)
  | tunit => tunit
  | tlet y e1 e2 => tlet y (subst x s e1) 
                         (if eq_id_dec x y then e2 else (subst x s e2))
  | tinl T e => tinl T (subst x s e)
  | tinr T e => tinr T (subst x s e)
  | tcase e1 i1 e2 i2 e3 => tcase (subst x s e1) i1 
                                  (if eq_id_dec x i1 then e2 else (subst x s e2)) i2
                                  (if eq_id_dec x i2 then e3 else (subst x s e3))
  | tnil T => tnil T
  | tcons e1 e2 => tcons (subst x s e1) (subst x s e2)
  | tlcase e1 e2 i1 i2 e3 => tlcase (subst x s e1) (subst x s e2) i1 i2
                                    (if eq_id_dec x i1 then e3 else 
                                       if eq_id_dec x i2 then e3 else
                                         subst x s e3)
  | tfix e => tfix (subst x s e)
  end.
...
Inductive value : tm -> Prop :=
  ...
  | v_list : forall v1 v2, value v1 -> value v2 -> value (tcons v1 v2)
